...
function getXdpNeighbors(&$total_rows = 0, $rowStart = 1, $rowEnd = 25, $xdpType = '', $hostId = '', $filterVal = '', $orderField = 'hostname', $orderDir = 'asc', $cactiOnly = 'on', $output = 'array') {
	...
	if ($xdpType) {
		...
		array_push($params,(strtolower($xdpType)));
	}

	if ($hostId > 0) {
		array_push($conditions,'(`host_id` = ? OR `neighbor_host_id` = ?)');
		array_push($params, $hostId,$hostId);
	}

	if ($cactiOnly == 'on') {
		array_push($conditions,'(`host_id` > 0 AND `neighbor_host_id` > 0)');
	}
	...
	if ($filterVal != '') {
		$searchArray  = ['hostname', 'neighbor_hostname', 'interface_name', 'interface_alias', 'neighbor_interface_name', 'neighbor_interface_alias', 'neighbor_platform', 'neighbor_software'];
		$searchFields = [];
		$searchParams = [];

		foreach ($searchArray as $f) {
			array_push($searchFields,"`$f` LIKE ?");
			array_push($searchParams,"%$filterVal%");
		}
...
function getXdpNeighborStats(&$total_rows = 0) {
	...
	if ($numHosts || $numInterfaces) {
		$total_rows++;
	}

	return (['hosts'=>$numHosts, 'interfaces'=> $numInterfaces, 'last_polled' => $lastPolled]);
}
...
function db_fetch_hash(& $result,$index_keys) {
	$assoc = [];             // The array we're going to be returning
...
function neighbor_build_snmp_auth($community, $version, $username, $password, $auth_proto, $priv_pass, $priv_proto, $context, $engineid) {
	...
	return ['auth' => $snmp_auth, 'version' => $version];
}
...
function neighbor_filter($action = '') {
	...
	?>
				...
				<td> Cacti Only</td>
				<td> <input type='checkbox' name='cacti_only' id='cacti_only' onChange='applyFilter()' <?php if (get_request_var('cacti_only') == 'on') {
					print 'checked';
				}?>></input></td>
				<td>
	...
	foreach ($rules as $rule) {
		...
		if ($rule_id) {
			...
			if (is_array($result) && count($result) > 0) {
				...
				$hash    = db_fetch_hash($result_vrf,['host_id', 'ip_address']);
...
function neighbor_build_vrf_data_query_sql($rule,$host_filter = '',$edge_filter = '') {
	...
	$tables     = ['plugin_neighbor_ipv4_cache as cache'];
	$table_join = ['LEFT JOIN plugin_neighbor_ipv4_cache cache ON cache.host_id=h.id'];
...
function neighbor_build_vrf_object_rule_item_filter($rule_id, $prefix = '') {
	...
	if ($rule_id) {
		$object_rule_items = db_fetch_assoc_prepared('SELECT * from plugin_neighbor_vrf_rule_items where rule_id=?',[$rule_id]);
...
function findCactiHost($hostName = null, $hostId = null) {
	...
	if ($hostId && $hostRecords = db_fetch_assoc_prepared('SELECT * from host where id = ?',[$hostId])) {
		$sorted = db_fetch_hash($hostRecords,['description']);
	} elseif ($hostRecords = db_fetch_assoc_prepared('SELECT * from host where LOWER(description) LIKE LOWER(?)',[$hostName])) {
		$sorted = db_fetch_hash($hostRecords,['description']);
	} elseif ($hostRecords = db_fetch_assoc_prepared('SELECT * from host where LOWER(description) LIKE LOWER(?)',[$strippedHost])) {
		$sorted = db_fetch_hash($hostRecords,['description']);
	}
...
function getCactiHostById($hostId) {
	if ($hostRecords = db_fetch_assoc_prepared('SELECT * from host where id = ?',[$hostId])) {
...
function findCactiInterface($hostId,$interface, $snmpIndex = null) {
	if (!$snmpIndex) {
		$snmpIndex = db_fetch_cell_prepared("SELECT snmp_index from host_snmp_cache where host_id = ? AND field_name = 'ifDescr' and field_value = ?",[$hostId, $interface]);
	}

	$cacheRecords = db_fetch_assoc_prepared('SELECT * from host_snmp_cache where host_id = ? AND snmp_index=?',[$hostId, $snmpIndex]);

	if ($cacheRecords) {
		$sorted = db_fetch_hash($cacheRecords,['snmp_index', 'field_name']);
...
function autoDiscoverHosts() {
	...
	if (sizeof($hosts)) {
		foreach ($hosts as $host) {
			...
			$existing = db_fetch_cell_prepared('SELECT host_id FROM plugin_neighbor_host WHERE host_id = ?', [$host['id']]);
	...
	db_execute_prepared("REPLACE INTO settings (name,value) VALUES ('plugin_neighbor_autodiscovery_lastrun', ?)", [time()]);
...
function inferIntSpeed($interface) {
	if (preg_match('/^Fa/',$interface)) {
		return (100);
	}

	if (preg_match('/^Gi/',$interface)) {
		return (1000);
	}

	if (preg_match('/^Te/',$interface)) {
		return (10000);
	}

	if (preg_match('/^Fo/',$interface)) {
		return (40000);
	}

	if (preg_match('/^One/',$interface)) {
		return (100000);
	}
}
...
function snipToDots($str,$len) {
	if (strlen($str) <= $len) {
		return '<span>' . htmlspecialchars($str, ENT_QUOTES, 'UTF-8') . '</span>';
	} else {
...
function pre_print_r($arr,$tag = '',$print = true) {
	if ($print) {
		...
		if ($tag) {
			print htmlspecialchars($tag, ENT_QUOTES, 'UTF-8') . "\n";
		}
...
if (!isset($_SERVER['argv']) || php_sapi_name() !== 'cli') {
	...
	$fields_neighbor_match_rule_item_edit = [
		'operation' => [
			'method'        => 'drop_array',
			'friendly_name' => __('Operation'),
			'description'   => __('Logical operation to combine rules.'),
			'array'         => isset($automation_oper) ? $automation_oper : [],
	...
	$fields_neighbor_graph_rule_item_edit = [
		'operation' => [
			'method'        => 'drop_array',
			'friendly_name' => __('Operation'),
			'description'   => __('Logical operation to combine rules.'),
		...
		'operator' => [
			...
			'friendly_name' => __('Operator'),
			'description'   => __('Operator.'),
			'array'         => (isset($automation_op_array) && isset($automation_op_array['display'])) ? $automation_op_array['display'] : [],
			'value'         => '|arg1:operator|',
			'on_change'     => 'toggle_operator()',
		],
		'pattern' => [
	...
	$fields_neighbor_graph_rules_edit1 = [
		'name' => [
			'method'        => 'textbox',
	...
	$fields_neighbor_graph_rules_edit2 = [
		'neighbor_type' => [
			'method'        => 'drop_array',
	...
	$fields_neighbor_graph_rules_edit3 = [
		'enabled' => [
			'method'        => 'checkbox',
	...
	$fields_neighbor_tree_rules_edit1 = [
		'name' => [
			'method'        => 'textbox',
			'friendly_name' => __('Name'),
		...
		'leaf_type' => [
			...
			'array'         => isset($automation_tree_item_types) ? $automation_tree_item_types : []
		],
		'host_grouping_type' => [
			'method'        => 'drop_array',
			...
			'array'         => isset($host_group_types) ? $host_group_types : [],
			'value'         => '|arg1:host_grouping_type|',
			'default'       => HOST_GROUPING_GRAPH_TEMPLATE,
		]
	];

	$fields_neighbor_tree_rules_edit2 = [
		'tree_item_id' => [
			'method'        => 'drop_tree',
	...
	$fields_neighbor_tree_rules_edit3 = [
		'enabled' => [
			'method'        => 'checkbox',
			'friendly_name' => __('Enable Rule'),
			'description'   => __('Check this box to enable this rule.'),
	...
	$fields_neighbor_tree_rule_item_edit = [
		'field' => [
			'method'        => 'drop_array',
		...
		'sort_type' => [
			...
			'array'         => isset($tree_sort_types) ? $tree_sort_types : [],
			],
		'propagate_changes' => [
			'method'        => 'checkbox',
...
$neighbor_interface_new_graph_fields = [
	'type'		                   => 'Type',
	'hostname'	                => 'A - Hostname',
...
$fields_neighbor_vrf_rules_edit1 = [
	'name' => [
		'method'        => 'textbox',
		'friendly_name' => __('Name'),
		'description'   => __('A useful name for this Rule.'),
...
$fields_neighbor_vrf_rules_edit2 = [
	'enabled' => [
		'method'        => 'checkbox',
		'friendly_name' => __('Enable Rule'),
		'description'   => __('Check this box to enable this rule.'),
