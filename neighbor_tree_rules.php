...
$neighbor_tree_rules_actions = array(
...
function neighbor_tree_rules_item_edit() {
	...
	if (!isempty_request_var('rule_type') && !isempty_request_var('item_id')) {
		...
		$item = db_fetch_row_prepared('SELECT * FROM neighbor_tree_rule_items WHERE id = ?', array(get_request_var('item_id')));
...
<table style='width:100%;text-align:center;'>
	...
	$rule = array();
	if (!isempty_request_var('id')) {
		$rule = db_fetch_row_prepared('SELECT * FROM neighbor_tree_rules WHERE id = ?', array(get_request_var('id')));
