...
switch (get_request_var('action')) {
	case 'save':
		neighbor_tree_rules_form_save();

		break;
	case 'actions':
		neighbor_tree_rules_form_actions();

		break;
	case 'item_movedown':
		neighbor_tree_rules_item_movedown();

		header('Location: neighbor_tree_rules.php?action=edit&id=' . get_request_var('id'));

		break;
	case 'item_moveup':
		neighbor_tree_rules_item_moveup();

		header('Location: neighbor_tree_rules.php?action=edit&id=' . get_request_var('id'));

		break;
	case 'item_remove':
		neighbor_tree_rules_item_remove();
	...
	case 'item_edit':
		top_header();
		neighbor_tree_rules_item_edit();
		bottom_footer();

		break;
	case 'remove':
		neighbor_tree_rules_remove();

		header('Location: neighbor_tree_rules.php');

		break;
	case 'edit':
		top_header();
		neighbor_tree_rules_edit();
		bottom_footer();
...
function neighbor_tree_rules_form_save() {
	if (isset_request_var('save_component_neighbor_tree_rule')) {
		// ================= input validation =================
		...
		header('Location: neighbor_tree_rules.php?header=false&action=edit&id=' . (empty($rule_id) ? get_request_var('id') : $rule_id));
		...
		if (is_error_message()) {
			header('Location: neighbor_tree_rules.php?header=false&action=item_edit&id=' . get_request_var('id') . '&item_id=' . (empty($item_id) ? get_request_var('item_id') : $item_id) . '&rule_type=' . AUTOMATION_RULE_TYPE_TREE_MATCH);
		} else {
			header('Location: neighbor_tree_rules.php?header=false&action=edit&id=' . get_request_var('id') . '&rule_type=' . AUTOMATION_RULE_TYPE_TREE_MATCH);
		}
	} elseif (isset_request_var('save_component_neighbor_tree_rule_item')) {
		// ================= input validation =================
		get_filter_request_var('id');
		get_filter_request_var('item_id');
		// ====================================================
		...
		if (is_error_message()) {
			header('Location: neighbor_tree_rules.php?header=false&action=item_edit&id=' . get_request_var('id') . '&item_id=' . (empty($neighbor_graph_rule_item_id) ? get_request_var('item_id') : $neighbor_graph_rule_item_id) . '&rule_type=' . AUTOMATION_RULE_TYPE_TREE_ACTION);
		} else {
			header('Location: neighbor_tree_rules.php?header=false&action=edit&id=' . get_request_var('id') . '&rule_type=' . AUTOMATION_RULE_TYPE_TREE_ACTION);
		}
	} else {
		raise_message(2);
		header('Location: neighbor_tree_rules.php?header=false');
	}
}
...
function neighbor_tree_rules_form_actions() {
	global $neighbor_tree_rules_actions;
	global $config;
		...
		if ($selected_items != false) {
			if (get_nfilter_request_var('drp_action') == AUTOMATION_ACTION_TREE_DELETE) { // DELETE
				...
				for ($i = 0; ($i < count($selected_items)); $i++) {
					...
					duplicate_neighbor_tree_rules($selected_items[$i], get_nfilter_request_var('name_format'));
				}
				...
				for ($i = 0; ($i < count($selected_items)); $i++) {
					...
					db_execute_prepared("UPDATE plugin_neighbor_tree_rules SET enabled='on' WHERE id = ?", [$selected_items[$i]]);
				}
				...
				for ($i = 0; ($i < count($selected_items)); $i++) {
					...
					db_execute_prepared("UPDATE plugin_neighbor_tree_rules SET enabled='' WHERE id = ?", [$selected_items[$i]]);
				}
			}
		}
	...
	foreach ($_POST as $var => $val) {
		if (preg_match('/^chk_([0-9]+)$/', $var, $matches)) {
			...
			$neighbor_tree_rules_list .= '<li>' . db_fetch_cell_prepared('SELECT name FROM plugin_neighbor_tree_rules WHERE id = ?', [$matches[1]]) . '</li>';
	...
	form_start('neighbor_tree_rules.php', 'neighbor_tree_rules_action');

	html_start_box($neighbor_tree_rules_actions[get_nfilter_request_var('drp_action')], '60%', '', '3', 'center', '');

	if (get_nfilter_request_var('drp_action') == AUTOMATION_ACTION_TREE_DELETE) { // DELETE
		print "<tr>
			<td class='textArea'>
				<p>" . __('Click \'Continue\' to delete the following Rule(s).') . "</p>
				<div class='itemlist'><ul>$neighbor_tree_rules_list</ul></div>
			</td>
		</tr>\n";
	} elseif (get_nfilter_request_var('drp_action') == AUTOMATION_ACTION_TREE_DUPLICATE) { // duplicate
		print "<tr>
	...
	if (!isset($neighbor_tree_rules_array)) {
	...
	print "<tr>
		<td class='saveRow'>
			...
			<input type='hidden' name='selected_items' value='" . (isset($neighbor_tree_rules_array) ? serialize($neighbor_tree_rules_array) : '') . "'>
...
function neighbor_tree_rules_item_movedown() {
	// ================= input validation =================
	get_filter_request_var('id');
	get_filter_request_var('item_id');
...
function neighbor_tree_rules_item_moveup() {
	// ================= input validation =================
	get_filter_request_var('id');
	get_filter_request_var('item_id');
	get_filter_request_var('rule_type');
...
function neighbor_tree_rules_item_remove() {
	// ================= input validation =================
	get_filter_request_var('item_id');
	get_filter_request_var('rule_type');
	// ====================================================
	...
	if (get_request_var('rule_type') == AUTOMATION_RULE_TYPE_TREE_MATCH) {
		...
		db_execute_prepared('DELETE FROM plugin_neighbor_tree_rule_items WHERE id = ?', [get_request_var('item_id')]);
	}
}

function neighbor_tree_rules_item_edit() {
	global $config;
	...
	if (isset_request_var('show_trees')) {
		if (get_request_var('show_trees') == '0') {
			kill_session_var('neighbor_tree_rules_show_trees');
		} elseif (get_request_var('show_trees') == '1') {
			$_SESSION['neighbor_tree_rules_show_trees'] = true;
		}
	}
	...
	if (!isempty_request_var('rule_type') && !isempty_request_var('item_id')) {
		if (get_request_var('rule_type') == AUTOMATION_RULE_TYPE_TREE_ACTION) {
			$item = db_fetch_row_prepared('SELECT * FROM neighbor_tree_rule_items WHERE id = ?', [get_request_var('item_id')]);

			if ($item['field'] != AUTOMATION_TREE_ITEM_TYPE_STRING) {
				?>
<table style='width:100%;text-align:center;'>
	<tr>
		<td class='textInfo' style='text-align:right;vertical-align:top;'><span class='linkMarker'>*</span><a class='linkEditMain' href='<?php print htmlspecialchars('neighbor_tree_rules.php?action=item_edit&id=' . (isset_request_var('id') ? get_request_var('id') : 0) . '&item_id=' . (isset_request_var('item_id') ? get_request_var('item_id') : 0) . '&rule_type=' . (isset_request_var('rule_type') ? get_request_var('rule_type') : 0) . '&show_trees=') . (isset($_SESSION['neighbor_tree_rules_show_trees']) ? '0' : '1'); ?>'><?php print(isset($_SESSION['neighbor_tree_rules_show_trees']) ? __('Don\'t Show') : __('Show')); ?> <?php print __('Created Trees'); ?></a><br>
		</td>
	</tr>
</table>
<br>
	...
	if (get_request_var('rule_type') == AUTOMATION_RULE_TYPE_TREE_MATCH) {
		...
		form_hidden_box('save_component_neighbor_tree_rule_item', '1', '');
	}
	form_save_button('neighbor_tree_rules.php?action=edit&id=' . get_request_var('id') . '&rule_type=' . get_request_var('rule_type'));
	print '<br>';

	// display list of matching trees
	if (!isempty_request_var('rule_type') && !isempty_request_var('item_id')) {
		if (get_request_var('rule_type') == AUTOMATION_RULE_TYPE_TREE_ACTION) {
			if (isset($_SESSION['neighbor_tree_rules_show_trees']) && ($item['field'] != AUTOMATION_TREE_ITEM_TYPE_STRING)) {
				if ($_SESSION['neighbor_tree_rules_show_trees']) {
					display_matching_trees(get_request_var('id'), AUTOMATION_RULE_TYPE_TREE_ACTION, $item, 'neighbor_tree_rules.php?action=item_edit&id=' . get_request_var('id') . '&item_id=' . get_request_var('item_id') . '&rule_type=' . get_request_var('rule_type'));
				}
			}
		}
	}
...
function neighbor_tree_rules_remove() {
	// ================= input validation =================
	get_filter_request_var('id');
	// ====================================================
	...
	if ((read_config_option('deletion_verification') == 'on') && (!isset_request_var('confirm'))) {
		top_header();
		form_confirm(__('Are You Sure?'), __("Are you sure you want to DELETE the Rule '%s'?", db_fetch_cell_prepared('SELECT name FROM plugin_neighbor_tree_rules WHERE id = ?', [get_request_var('id')])), 'neighbor_tree_rules.php', 'neighbor_tree_rules.php?action=remove&id=' . get_request_var('id'));
		bottom_footer();
		exit;
	}
...
function neighbor_tree_rules_edit() {
	global $config;
	global $fields_neighbor_tree_rules_edit1, $fields_neighbor_tree_rules_edit2, $fields_neighbor_tree_rules_edit3;
	...
	if (isset_request_var('show_hosts')) {
		if (get_request_var('show_hosts') == '0') {
			kill_session_var('neighbor_tree_rules_show_objects');
		} elseif (get_request_var('show_hosts') == '1') {
			$_SESSION['neighbor_tree_rules_show_objects'] = true;
		}
	}
...
<table style='width:100%;text-align:center;'>
	<tr>
		<td class='textInfo right' style='vertical-align:top;'><span class='linkMarker'>*</span><a class='linkEditMain' href='<?php print htmlspecialchars('neighbor_tree_rules.php?action=edit&id=' . (isset_request_var('id') ? get_request_var('id') : 0) . '&show_hosts=') . (isset($_SESSION['neighbor_tree_rules_show_objects']) ? '0' : '1'); ?>'><?php print(isset($_SESSION['neighbor_tree_rules_show_objects']) ? __('Don\'t Show') : __('Show')); ?> <?php print __('Eligible Objects'); ?></a><br>
		</td>
	</tr>
</table>
		<?php
	...
	if (!isempty_request_var('id')) {
		$rule         = db_fetch_row_prepared('SELECT * FROM neighbor_tree_rules WHERE id = ?', [get_request_var('id')]);
	...
	form_start('neighbor_tree_rules.php', 'form_neighbor_tree_rule_edit');

	html_start_box($header_label, '100%', true, '3', 'center', '');

	if (!isempty_request_var('id')) {
		// display whole rule
		$form_array = $fields_neighbor_tree_rules_edit1 + $fields_neighbor_tree_rules_edit2 + $fields_neighbor_tree_rules_edit3;
	} else {
		// display first part of rule only and request user to proceed
		$form_array = $fields_neighbor_tree_rules_edit1;
	}
	...
	form_hidden_box('save_component_neighbor_tree_rule', '1', '');
	...
	form_save_button('neighbor_tree_rules.php', 'return');

	print '<br>';

	if (!empty($rule['id'])) {
		// display list of matching hosts
		if (isset($_SESSION['neighbor_tree_rules_show_objects'])) {
			if ($_SESSION['neighbor_tree_rules_show_objects']) {
				if ($rule['leaf_type'] == TREE_ITEM_TYPE_HOST) {
					display_matching_hosts($rule, AUTOMATION_RULE_TYPE_TREE_MATCH, 'neighbor_tree_rules.php?action=edit&id=' . get_request_var('id'));
				} elseif ($rule['leaf_type'] == TREE_ITEM_TYPE_GRAPH) {
					display_matching_graphs($rule, AUTOMATION_RULE_TYPE_TREE_MATCH, 'neighbor_tree_rules.php?action=edit&id=' . get_request_var('id'));
				}
			}
		}
	}
	...
	function applyTreeChange() {
		strURL  = 'neighbor_tree_rules.php?header=false&action=edit&id=' + $('#id').val();
...
function neighbor_tree_rules() {
	global $neighbor_tree_rules_actions, $config, $item_rows;
	global $neighbor_tree_item_types, $host_group_types;
	...
	html_start_box(__('Tree Rules'), '100%', '', '3', 'center', 'neighbor_tree_rules.php?action=edit');

	?>
	<tr class='even'>
		<td>
			<form id='form_automation' action='neighbor_tree_rules.php'>
	...
	?>
			...
			function applyFilter() {
				strURL = 'neighbor_tree_rules.php' +
					'?status='+$('#status').val() +
					'&filter='+$('#filter').val() +
					'&rows='+$('#rows').val() +
					'&header=false';
			...
			function clearFilter() {
				strURL = 'neighbor_tree_rules.php?clear=1&header=false';
				loadPageNoHeader(strURL);
			}
	...
	$nav = html_nav_bar('neighbor_tree_rules.php?filter=' . get_request_var('filter'), MAX_DISPLAY_PAGES, get_request_var('page'), $rows, $total_rows, 11, __('Tree Rules'), 'page', 'main');

	form_start('neighbor_tree_rules.php', 'chk');
	...
	$display_text = [
		'name'               => ['display' => __('Rule Name'),      'align' => 'left',  'sort' => 'ASC'],
		'id'                 => ['display' => __('ID'),             'align' => 'right', 'sort' => 'ASC'],
		'tree_name'          => ['display' => __('Hook into Tree'), 'align' => 'left',  'sort' => 'ASC'],
		'subtree_name'       => ['display' => __('At Subtree'),     'align' => 'left',  'sort' => 'ASC'],
		'leaf_type'          => ['display' => __('This Type'),      'align' => 'left',  'sort' => 'ASC'],
		'host_grouping_type' => ['display' => __('Using Grouping'), 'align' => 'left',  'sort' => 'ASC'],
		'enabled'            => ['display' => __('Enabled'),        'align' => 'right', 'sort' => 'ASC']
	];

	html_header_sort_checkbox($display_text, get_request_var('sort_column'), get_request_var('sort_direction'), false);

	if (sizeof($neighbor_tree_rules)) {
		foreach ($neighbor_tree_rules as $neighbor_tree_rule) {
			$tree_item_type_name     = ((empty($neighbor_tree_rule['leaf_type'])) ? '<em>' . __('None') . '</em>' : $neighbor_tree_item_types[$neighbor_tree_rule['leaf_type']]);
			$subtree_name            = ((empty($neighbor_tree_rule['subtree_name'])) ? '<em>' . __('ROOT') . '</em>' : $neighbor_tree_rule['subtree_name']);
			$tree_host_grouping_type = ((empty($host_group_types[$neighbor_tree_rule['host_grouping_type']])) ? '' : $host_group_types[$neighbor_tree_rule['host_grouping_type']]);
			form_alternate_row('line' . $neighbor_tree_rule['id'], true);

			form_selectable_cell(filter_value($neighbor_tree_rule['name'], get_request_var('filter'), 'neighbor_tree_rules.php?action=edit&id=' . $neighbor_tree_rule['id'] . '&page=1'), $neighbor_tree_rule['id']);
			form_selectable_cell($neighbor_tree_rule['id'], $neighbor_tree_rule['id'], '', 'text-align:right');
			form_selectable_cell($neighbor_tree_rule['tree_name'], $neighbor_tree_rule['id']);
			form_selectable_cell($subtree_name, $neighbor_tree_rule['id']);
			form_selectable_cell($tree_item_type_name, $neighbor_tree_rule['id']);
			form_selectable_cell($tree_host_grouping_type, $neighbor_tree_rule['id']);
			form_selectable_cell($neighbor_tree_rule['enabled'] ? __('Enabled') : __('Disabled'), $neighbor_tree_rule['id'], '', 'text-align:right');
			form_checkbox_cell($neighbor_tree_rule['name'], $neighbor_tree_rule['id']);
	...
	draw_actions_dropdown($neighbor_tree_rules_actions);
